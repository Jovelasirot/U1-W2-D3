...
for (let i = 0; i < starWarsCharacters.length; i++) {
  const called = starWarsCharacters[i].name;
  charactersNames.push(called);
}
...
for (let i = 0; i < starWarsCharacters.length; i++) {
  const female = starWarsCharacters[i].gender;
  if (female === "female") {
...
}  if (totalMass >500 && totalMass <700 ) {
  console.log("Ship is half loaded"),}
   if (totalMass > 700 && totalMass <900 ) {
  console.log("Warning: Load is over 700")
}  if (totalMass > 900 && totalMass <1000 ) {
...
for (let i = 0; i < starWarsCharacters.length; i++) {
  const robot = starWarsCharacters[i].gender;
  if (robot === "n/a") {
    starWarsCharacters.splice(starWarsCharacters[i].gender, 1, "Robot");
  }
}
